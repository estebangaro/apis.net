            ...
            return Task<object>.Run(() =>
            {
                ...
                using (StreamReader lector = new StreamReader(readStream))
                {
                    XElement equipos = XElement.Parse(
                            lector.ReadToEnd());
                    coleccion = new List<Entity_Framework_CF.Equipo>();
                    coleccion.AddRange(
                            equipos.Elements().Select(
                                    equipo =>
                                    {
                                        ...
                                        return new Equipo
                                        {
                                            ...
                                            Jugadores = equipo.Element("Jugadores")!= null && equipo.Element("Jugadores").HasElements ?
                                                equipo.Element("Jugadores").Elements().Select(
                                                        jugador =>
                                                        {
                                                            ...
                                                            return new Jugador
                                                            {
                                                                ...
                                                                Posicion = (Entity_Framework_CF.POSICION_SOCCER)
